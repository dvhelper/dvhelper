...
dvhelper D:\Movies -o
```
整理并重命名D:\Movies目录下的影片文件夹，根据预设的映射表（actress_alias.json）进行重命名。
...
2. **organize功能说明**：使用`-o`或`--organize`参数时，程序会根据映射表（actress_alias.json）重命名文件夹，映射表中定义了演员名称的别名，程序会将文件夹名称中的演员名称替换为固定名称。
