...
**DV Helper** 是一款影片信息搜索和整理工具，能够自动从网站抓取影片信息，下载封面图片、剧照、预告片，生成标准的NFO文件，并按照演员分类整理影片文件。
...
选项:
  ...
  -o, --organize        （模拟操作）整理并重命名指定目录下的影片文件夹
...
```
扫描D:\Movies目录及其一级子目录中的视频文件，下载剧照和预告片，并强制重新登录。

#### 7. 整理并重命名影片文件夹（模拟操作）
```bash
dvhelper D:\Movies -o
```
以模拟操作方式整理并重命名D:\Movies目录下的影片文件夹，不会实际执行文件操作。
...
4. **资源下载**：下载影片封面图片并进行裁剪处理。如果启用了-g选项，还会下载剧照和预告片。
   - 剧照会以gallery_01.jpg、gallery_02.jpg等格式保存
   - 预告片会以影片编号_trailer.扩展名格式保存，自动从URL中提取合适的扩展名
5. **文件生成**：创建标准的NFO文件，包含影片的完整元数据。
6. **文件整理**：按照演员名称分类创建目录，并将影片文件移动到相应目录。

### 文件夹整理功能 (--organize)

使用`-o`或`--organize`选项时，程序会执行以下操作：

1. **递归查找**：从指定目录开始，递归查找所有子文件夹。
2. **映射表匹配**：根据预设的文件夹名称映射表（在actress_alias.json中定义）查找需要重命名的文件夹。
3. **忽略特定文件夹**：当找到映射表中匹配的文件夹名称时，该文件夹会被忽略，不再继续递归查找其子文件夹。
4. **模拟操作**：目前此功能处于模拟阶段，只会输出操作日志，不会实际执行文件重命名或移动操作。
5. **日志输出**：详细记录每个处理步骤，包括正在处理的文件夹路径和目标路径，但不会执行实际的文件操作。
...
2. **organize功能说明**：使用`-o`或`--organize`参数时，程序目前处于模拟操作阶段，只会输出操作日志，不会实际执行文件重命名或移动操作。
3. **登录操作**：使用`-l`参数强制重新登录时，程序会打开Chrome浏览器，用户需要手动完成登录操作。
4. **影片命名**：为了提高识别率，请确保影片文件名包含正确的影片编号。
5. **支持的格式**：程序支持多种常见视频格式，包括但不限于MP4、MKV、AVI、WMV等。
6. **性能提示**：批量处理大量视频文件时，建议适当设置搜索深度，避免处理过多无关文件。
7. **剧照和预告片下载**：使用`-g`选项时，程序会尝试下载剧照和预告片，请注意这可能会增加处理时间和网络流量。预告片格式会自动从URL中提取，支持多种常见视频格式。
